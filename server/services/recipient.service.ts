...
export async function updateStatus(recipientId: number, status: "pending" | "signed" | "declined"): Promise<Recipient> {
    return recipientRepo.updateRecipientStatus(recipientId, status);
}

export async function inviteRecipient(documentId: string, recipientId: number): Promise<void> {
