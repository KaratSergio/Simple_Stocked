...
export default function Sidebar() {
    ...
    return (
        <aside className="w-44 bg-teal-900 text-white flex flex-col p-4">
            {menu.map(item => {
                const isActive =
                    item.href === '/page'
                        ? pathname === '/page'
                ...
                return (
                    <Link key={item.key} href={item.href}>
                        <button
                            className={`mb-2 w-[144px] px-3 py-2 text-left rounded flex items-center gap-2 transition-colors duration-200
